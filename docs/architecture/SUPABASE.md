...
Registration uses only the server-generated access token; no codeword is collected or stored. A separate random recovery code is generated server-side after account creation. Its raw value is shown once to the user before a session is created; the user signs in with the generated token when continuing to the checks screen. `public.auth_recovery_codes` stores only the recovery code's SHA-256 digest. RLS is enabled and direct access is limited to the service-role client. This schema supports a future protected support recovery process; no public recovery endpoint is exposed.
